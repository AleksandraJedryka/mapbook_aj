...
from mapbook_lib.controller import update_users
...
def user_details(users_data:list):
    ...
    label_imie_szczegoly_obiektu_wartosc.config(text=users_data[i].imie)
...
def update_user(users_data:list,i):
    ...
    button_dodaj_obiekt.config(text="Zapisz zmiany", command=lambda: add_user(users_data, i))
    entry_name.delete(0, END)
    entry_lokalizacja.delete(0, END)
    entry_img_url.delete(0, END)
    entry_posty.delete(0, END)
